...
def _tool_prompt(label: str, detail: str, history: list[dict] | None, role: str = "auto") -> str:
    ...
    if role == "send":
        return head + (
            ...
            "  **노션이면** 앞 결과물을 **새 페이지로 생성**한다 — 제목은 핵심을 한 줄로, 본문은 요약을 "
            "문단·불릿으로 정리해 넣는다. 만들 위치는 [이번 단계]의 detail에 적힌 페이지/DB 이름으로 "
            "**먼저 검색(search)** 해서 그 페이지를 부모로 삼고, detail이 비었으면 접근 가능한 페이지 "
            "중 적절한 곳에 만든다. **반드시 페이지 생성 도구를 실제로 호출**하고, 텍스트만 출력한 채 "
            "끝내지 마라.\n"
