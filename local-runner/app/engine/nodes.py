...
def _tool_prompt(label: str, detail: str, history: list[dict] | None, role: str = "auto") -> str:
    ...
    if role == "send":
        return head + (
            ...
            "만든다 — 예: [제휴 제안]), 그 뒤에 메일 핵심을 한 줄로, **끝에 앞 단계 메일 수신일을 "
            "괄호로 덧붙인다** — 예: '[제휴 제안] MYFAVE 라이브커머스 공동 기획전 (2026-07-17)'. "
            "목록에서 열어보지 않고도 유형과 언제 온 메일인지 알 수 있게 한다. 본문은 맨 위에 '수신일: …'을 "
            "적고, 그 아래를 **굵은 소제목으로 구획을 나눠**(예: 제안사 / 제안 내용 / 조건 / 기대 효과 / "
